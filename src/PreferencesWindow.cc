...
PreferencesWindow::PreferencesWindow(Preferences* thePreferences) {
...
void PreferencesWindow::highlight() {
...
void PreferencesWindow::run() {
 ...
 // Make labels for the above GtkEntrys
  ...
  list<string> checked_exif_selections = thePreferences->get_checked_exif_selections();
  list<string> text_exif_selections = thePreferences->get_text_exif_selections();
  BOOST_FOREACH(string choice, exif_choices) {
    GtkWidget *check_button = gtk_check_button_new_with_label(choice.c_str());
    if ( find(checked_exif_selections.begin(), checked_exif_selections.end(), choice) !=
        checked_exif_selections.end() ||
        find(text_exif_selections.begin(), text_exif_selections.end(), choice) !=
...
void PreferencesWindow::apply() {
  ...
  list<string> checked_exif_selections = get_checked_exif_selections();
  list<string> text_exif_selections = get_text_exif_selections();
  thePreferences -> set_exif_selections(checked_exif_selections, text_exif_selections);
  thePreferences -> writeback();
}

list<string> PreferencesWindow::get_checked_exif_selections() {
  list<string> selections;
  BOOST_FOREACH(GtkWidget *check_button, exif_check_buttons) {
    if (gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(check_button))) {
      string exif_name = gtk_button_get_label(GTK_BUTTON(check_button));
...
list<string> PreferencesWindow::get_text_exif_selections() {
  list<string> selections;
  return selections;
} 

void PreferencesWindow::quit() {
  gtk_widget_destroy(window);
}

void PreferencesWindow::accept_button_clicked() {
  apply();
  quit();
}

void PreferencesWindow::apply_button_clicked() {
  apply();
}

void PreferencesWindow::cancel_button_clicked() {
  quit();
}

/* static */ void
PreferencesWindow::accept_button_clicked_cb(GtkWidget* widget, gpointer callback_data) {
...
/* static */ void
PreferencesWindow::cancel_button_clicked_cb(GtkWidget* widget, gpointer callback_data) {
  PreferencesWindow *preferencesWindow = WidgetRegistry<PreferencesWindow>::get_object(widget);
  preferencesWindow->cancel_button_clicked();
}

/* static */ void
PreferencesWindow::apply_button_clicked_cb(GtkWidget* widget, gpointer callback_data) {
  PreferencesWindow *preferencesWindow = WidgetRegistry<PreferencesWindow>::get_object(widget);
