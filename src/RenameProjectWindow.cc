...
RenameProjectWindow::RenameProjectWindow(Preferences *preferences_,
...
void RenameProjectWindow::quit() {
  gtk_widget_destroy(GTK_WIDGET(window));
}

void RenameProjectWindow::run() {
...
string RenameProjectWindow::get_old_project_name() {
  if (NULL == first_radio_button) {
    return "";
  }
  ...
  for (GSList *p = radio_buttons; p != NULL; p = g_slist_next(p)) {
    ...
    if (gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(radio_button))) {
      ...
      string project_name = gtk_label_get_text(label);
	return project_name;
    }
  }
  return "";
...
void RenameProjectWindow::set_error_label(string text) {
  gtk_label_set_markup(GTK_LABEL(error_label),
      ("<span color=\"red\">" + text + "</span>").c_str());
  gtk_widget_show(GTK_WIDGET(error_label));
}

void RenameProjectWindow::accept() {
  bool error_occurred = false;
  string error_string;
  string old_project_name = get_old_project_name();
  ...
  string new_project_name = gtk_entry_get_text(GTK_ENTRY(new_name_entry));
...
/* static */ void RenameProjectWindow::accept_button_clicked_cb(GtkWidget *widget, gpointer callback_data) {
  RenameProjectWindow *renameProjectWindow =
      WidgetRegistry<RenameProjectWindow>::get_object(widget);
  renameProjectWindow->accept();
}

/* static */ void RenameProjectWindow::quit_button_clicked_cb(GtkWidget *widget, gpointer callback_data) {
  RenameProjectWindow *renameProjectWindow =
      WidgetRegistry<RenameProjectWindow>::get_object(widget);
